...
var f = controls.f.value; //częstotliwość próbkowania
...
for(var i=1;i<=czas;i++)
{
    ...
    var Vz = (doc_pos - pozY_bramki[i-1] - s_ham)/(Tx - t_ham); //prędkość zadania w danym momencie czasu
    var az = KT*((Vz - V_pilki[i-1])/Txp); //przyspieszenie zadane na podst, prędkości zadanej
    suma = suma + e[i];
    var de = e[i] - e[i-1];
    var a_n = przys_bramki[i-1] + k * (kp * (kb * e[i]) + ki * suma * dt + kd * de * dt);
    przys_bramki[i] = a_n;
    e.push(az-przys_bramki[i]);
